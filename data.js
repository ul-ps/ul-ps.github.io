...
class AppStore {
    ...
    async initFirebase() {
        try {
            // Firebase config is inlined here to avoid file:// import issues in Electron
    ...
    migrate() {
        ...
        if (!this.data.partnerLogs) this.data.partnerLogs = [];
        if (!this.data.settings.branding) this.data.settings.branding = JSON.parse(JSON.stringify(INITIAL_DATA.settings.branding));
        if (!this.data.settings.productCategories) this.data.settings.productCategories = INITIAL_DATA.settings.productCategories;
        if (!this.data.settings.expenseCategories) this.data.settings.expenseCategories = INITIAL_DATA.settings.expenseCategories;
        if (!this.data.settings.currencies) this.data.settings.currencies = INITIAL_DATA.settings.currencies;
        if (!this.data.settings.orderSeq) this.data.settings.orderSeq = this.data.orders.length;
        ...
        this.data.products.forEach(p => { if (!p.category) p.category = 'عام'; });
        ...
        this.data.orders.forEach(o => {
            ...
            if (o.deliveryCompanyId && (o.status === 'SCHEDULED' || o.status === 'COMPLETED')) {
                ...
                if (orderDate >= startOfTomorrow) {
                    ...
                    // We clear the isScheduled flag for active shipments to prevent confusion
