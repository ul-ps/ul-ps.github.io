...
class AppStore {
    ...
    async initFirebase() {
        try {
            ...
            // Use > instead of !== to ensure we only reload if cloud is NEWER.
            // If local is newer (e.g. from a recent offline edit or migrate), 
            // we skip reload and let syncToCloud handle the update.
            if (!this._pendingSync && cloudTimestamp && cloudTimestamp > localTimestamp) {
                console.log('☁️ Cloud data is newer — updating localStorage and reloading...');
