...
class AppStore {
    ...
    async initFirebase() {
        try {
            ...
            // SAFETY CHECK: If cloud has data and local is essentially empty, ALWAYS prefer cloud.
            const isLocalEmpty = (this.data.orders || []).length === 0 && (this.data.products || []).length === 0;
            const isCloudNotEmpty = cloudData && ((cloudData.orders || []).length > 0 || (cloudData.products || []).length > 0);

            if (isCloudNotEmpty && (isLocalEmpty || cloudTimestamp > localTimestamp)) {
                console.log('☁️ Cloud data prioritized — updating localStorage and reloading...');
            ...
            onValue(this.cloudRef, (snap) => {
                ...
                // Only reload if the incoming data is strictly newer than our local memory.
                if (liveTimestamp && liveTimestamp > (this.data.lastUpdated || 0)) {
                    console.log('☁️ Live update is newer than local — reloading...');
                    this.data = liveData;
    ...
    // Saves only to localStorage (used for structural migrations)
    saveLocal() {
        localStorage.setItem(DB_KEY, JSON.stringify(this.data));
    }
    ...
    async factoryReset() {
        ...
        if (this._dbSet && this._cloudRef) {
            try {
                ...
                console.log('☁️ Factory Reset synced to Cloud ✅');
            } catch (e) {
                console.error('☁️ Factory Reset Cloud Sync FAILED ❌:', e.message);
            }
        }
        location.reload();
    }
    ...
    migrate() {
        ...
        if (modified) {
            console.log("🛠️ Data migrated/fixed, saving local...");
            this.saveLocal();
        }
    }
    ...
    recalculateGlobalFunds() {
        ...
        // 3. Round to 2 decimal places to avoid floating point noise (0.1 + 0.2 = 0.30000000000000004)
        this.getCurrencies().forEach(c => {
            if (this.data.funds[c.code]) {
                this.data.funds[c.code] = Math.round(this.data.funds[c.code] * 100) / 100;
            }
        });
...
const UI = {
    ...
    updateDocumentTitle: () => {
        ...
        // Logic: Keep the first part of the title (e.g., "لوحة التحكم") and append the brand name
        if (currentTitle.includes('|')) {
            const pagePart = currentTitle.split('|')[0].trim();
            document.title = `${pagePart} | ${brandName}`;
        } else {
            // If no separator, just append it
    ...
    injectSidebar: () => {
        ...
        const logoHtml = brand.logo
            ? `<img src="${brand.logo}" class="sidebar-logo">`
            : '';
        const nameHtml = !brand.logo ? `<div class="sidebar-name">${brand.name}</div>` : '';

        const sidebarHTML = `
            <div class="sidebar-header">
                <button class="sidebar-close-btn mobile-only" onclick="UI.toggleSidebar(false)">
            ...
            <ul class="sidebar-menu">
                ...
                <li style="margin-top: auto; border-top: 1px solid #f1f5f9; padding-top: 10px;">
        ...
        if (!document.querySelector('.sidebar')) {
            ...
            toggleBtn.onclick = () => UI.toggleSidebar();
            document.body.appendChild(toggleBtn);

            // Create Overlay
            const overlay = document.createElement('div');
            overlay.className = 'sidebar-overlay';
            overlay.onclick = () => UI.toggleSidebar(false);
            ...
            const main = document.querySelector('body > :not(.sidebar):not(.menu-toggle-btn):not(.sidebar-overlay)');
            ...
            <head>
                ...
                <style>
                    ...
                    * { box-sizing: border-box; }
                    ...
                    .print-actions { margin-top: 20px; display: flex; gap: 10px; justify-content: center; }
